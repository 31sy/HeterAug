...
import networks
from datasets.datasets_c import LIPDataValSet
...
def get_arguments():
    ...
    parser.add_argument("--severity-level", type=int, default=5)
...
def main():
    ...
    miou_c = 0
    for severity in range(args.severity_level):
       

        # Data loader
        lip_test_dataset = LIPDataValSet(args.data_dir, 'val', crop_size=input_size, transform=transform, flip=args.flip, corruption_name=args.corruption_type, severity=severity+1)
        ...
        print('corruption_name:', args.corruption_type,'severity-level:',severity+1,'mIoU:',mIoU['Mean IU'])

        miou_c += mIoU['Mean IU']

    miou_c = miou_c / args.severity_level

    print('corruption_name:', args.corruption_type,'severity-level:',severity+1,'avg_mIoU:',miou_c)
